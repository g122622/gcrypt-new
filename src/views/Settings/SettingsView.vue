<template>
    ...
    <Teleport to="#ActionToolBar">
        <ActionToolBarBase ToolbarTitle="设置">
            ...
            <IconBtn icon="mdi-content-save" @click="settingsStore.saveSettings()"
                tooltip="手动保存设置（app会自动保存设置，一般情况不用点）" />
    ...
    <!-- 搜索框 -->
    <AdvancedTextField v-model:current-input="searchWord" label="搜索..." prepend-inner-icon="mdi-magnify"
        density="compact" style="border-radius: 15px;overflow: hidden;margin: 15px;margin-bottom: 0px;">
    </AdvancedTextField>
                    ...
                    <v-list-item v-for="item in extractByCat(category)" :key="item.name" :title="item.title"
                        ...
                        <template v-slot:append>
                            ...
                            <div v-else-if="item.type === SettingTypes.img" style="width:155px;">
                                <v-file-input show-size accept="image/png, image/jpeg, image/bmp" label="选择图片"
                                    @update:modelValue="(files) => {
                                        handleFile(files, item.name);
                                    }" density="compact" counter></v-file-input>
                            </div>
                            <!-- 滑动条 -->
                            ...
                            <div v-else-if="item.type === SettingTypes.text">
                                <IconBtn icon="mdi-open-in-new" tooltip="打开并编辑" variant="plain"
                                    @click="handleText(item)">
                                </IconBtn>
                            </div>
                            <!-- 颜色 -->
                            <div v-else-if="item.type === SettingTypes.color"
