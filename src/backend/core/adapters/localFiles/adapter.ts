/**
 * File: \src\backend\core\adapters\localFiles\adapter.ts
 * Project: Gcrypt
...
class LocalFileAdapter extends Disposable implements IAdapter {
    ...
    public async readFile(filename, dir?: Addr): Promise<Buffer> {
        ...
        if (dir) {
            return await VFS.readFile(path.join(dir.toPathStr(), filename));
        } else {
            return await VFS.readFile(path.join(this.currentDirectory.toPathStr(), filename));
        }
    }
    ...
    public async writeFile(filename: string, data: Buffer | string, dir?: Addr): Promise<string> {
        ...
        await VFS.writeFile(path.join(this.currentDirectory.toPathStr(), filename), bufData);
    ...
    public mkdir = async function (folderName: string, dir?: Addr): Promise<void> {
        ...
        await VFS.mkdir(path.join(this.currentDirectory.toPathStr(), folderName));
    ...
    public deleteFile = async (filename: string, dir?: Addr) => {
        ...
        // 如果是文件，则直接删除即可
        if (this.currentFileTable.items.find(item => item.name === filename).type === "file") {
            await VFS.unlink(path.join(this.currentDirectory.toPathStr(), filename));
        } else {
            // 如果是文件夹，则需要递归删除。
            await VFS.rm(path.join(this.currentDirectory.toPathStr(), filename), { recursive: true });
        }
