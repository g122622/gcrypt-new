            ...
            <v-main style="display: block; height: 100%;width: 100%;overflow-x: hidden;">
                ...
                {{ item.name }}
                    ...
                    <div v-else-if="currentFileTableForRender.length === 0 && !isLoading"
                        ...
                        <img src="/public/assets/fileMgr/404.png" style="width:270px;" />
                        当前目录下没有文件
                    </div>
            ...
            return new Promise((resolve, reject) => {
                ...
                reader.onload = async function (evt) {
                    ...
                    try {
                        ...
                        fileKey = await props.adapter.writeFile(
                            'path' in file ? getFileName(file.path, true) : (file as any).name,
                            dataBuf)
                        // 处理缩略图相关逻辑
                        if (isElectron() && options.useThumbnails && props.adapter.setExtraMeta) {
