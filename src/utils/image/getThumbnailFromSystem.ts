...
/**
 * 从本地系统COM Surrogate获取给定文件的缩略图，JPEG压缩处理后返回图像Base64编码
 * @param filePath 文件在本地文件系统上的路径
 */
const getThumbnailFromSystem = async (
    filePath: string,
    options: { height: number; width: number; quality: number }
): Promise<string> => {
    const res = await (
        await electron.nativeImage.createThumbnailFromPath(filePath, { height: options.height, width: options.width })
    )
